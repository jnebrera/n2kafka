...
static const char MERAKI_SECRETS[] = // *INDENT-OFF*
		"{"
		/* "\"meraki-secrets\": {" */
		"\"redborder\": { "
		"\"sensor_name\": \"meraki1\" "
...
static void MerakiDecoder_nulls() {
	...
	static const struct checkdata_array checkdata = {
			...
			.size = sizeof(checkdata_array) /
				sizeof(checkdata_array[0]),
	};

	MerakiDecoder_test_base(NULL, MERAKI_SECRETS, MERAKI_MSG, &checkdata);
}
...
static void MerakiDecoder_bad_location() {
	...
	MerakiDecoder_test_base(NULL,
				MERAKI_SECRETS,
				MERAKI_MSG_BAD_FORMAT_LOCATION,
				&checkdata);
}
