...
static void
checkMSE10Decoder_valid_enrich(struct mse_array *notifications_array) {
	/* No database -> output == input */
	assert_int_equal(notifications_array->size, 1);
	assert_int_equal(notifications_array->data[0].string_size,
...
static void
checkMSE10Decoder_novalid_enrich(struct mse_array *notifications_array) {
	/* No database -> output == input */
	assert_int_equal(notifications_array->size, 1);
...
static void checkMSE10Decoder_multi(struct mse_array *notifications_array) {
	...
	assert_int_equal(notifications_array->data[0].string_size,
			 strlen(notifications_array->data[0].string));
	json_t *ret1 = json_loads(
			notifications_array->data[0].string, 0, &jerr);
	assert_non_null(ret1);
	const int unpack_rc1 = json_unpack_ex(ret1,
					      &jerr,
					      0,
					      "{s:[{s:s,s:s,s:i}]}",
					      "notifications",
	...
	assert_int_equal(notifications_array->data[1].string_size,
			 strlen(notifications_array->data[1].string));

	json_t *ret2 = json_loads(
			notifications_array->data[1].string, 0, &jerr);
	assert_non_null(ret2);
	const int unpack_rc2 = json_unpack_ex(ret2,
					      &jerr,
					      0,
					      "{s:[{s:s,s:s,s:i}]}",
					      "notifications",
...
/// @TODO use a for loop
static void
checkMSE10Decoder_empty_array(struct mse_array *notifications_array) {
	/* No database -> output == input */
