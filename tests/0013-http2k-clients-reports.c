...
static void validate_report0(json_t *config,
	...
	for (i = 0; i < vsize; ++i) {
		...
		if (validators[i].sync_msgs) {
			consume_sync_msgs(&db,
					  validators[i].consume_ctx,
					  TEST_N2KAFKA_ID,
					  validators[i].sync_msgs);
		}

		if (validators[i].org_db_cb_bef_report) {
			validators[i].org_db_cb_bef_report(
					&db,
					validators[i].org_db_cb_bef_report_ctx);
		}

		if (validators[i].validate_cb) {
			struct kafka_message_array *ma =
					validators[i].clean
							? organization_db_clean_consumed(
									  &db,
									  test_timestamp,
									  &timer_interval,
									  &clean_interval,
									  TEST_N2KAFKA_ID)
							: organization_db_interval_consumed(
									  &db,
									  test_timestamp,
									  &timer_interval,
									  &clean_interval,
									  TEST_N2KAFKA_ID);
			validators[i].validate_cb(
					ma, validators[i].validate_cb_ctx);

			free(ma);
		}

		if (validators[i].org_db_cb_aft_report) {
			validators[i].org_db_cb_aft_report(
					&db,
...
static void
validate_zero_clients_reports0(struct kafka_message_array *ma, void *ctx) {
	(void)ctx;
	assert(ma);
	assert(0 == ma->count);
}
...
static void validate_msg(json_t *json) {
	...
	const int unpack_ex = json_unpack_ex(json,
					     &jerr,
					     0,
					     "{s:s,s:s,s:s}",
					     "monitor",
...
/** Validate all messages in a queue calling cb for each message
  ...
  */
static void
validate_msgs(struct kafka_message_array *ma,
	      void (*key_cb)(const char *key, int keylen, void *cb_ctx),
	      void (*cb)(json_t *msg, void *cb_ctx),
	      void *cb_ctx) {
	json_error_t jerr;
	size_t i;
	...
	for (i = 0; i < ma->count; ++i) {
		json_t *msg = json_loadb(
				ma->msgs[i].payload, ma->msgs[i].len, 0, &jerr);
		if (NULL == msg) {
			rdlog(LOG_ERR,
			      "Couldn't load msg %zu: %s",
			      i,
			      jerr.text);
			assert(0);
		}
...
static void validate_zero_bytes_reports() {
	...
	for (i = 0; i < n_uuids; ++i) {
		...
		json_object_set_new(
				uuids, uuid, new_config_uuid(json_object(), 0));
	}
...
/** Do something foreach client, where client is a numeric string from 0 to
  n_uuids */
static void
foreach_numeric_client(organizations_db_t *db,
		       size_t n_uuids,
		       void (*cb)(size_t i,
				  organization_db_entry_t *organization,
				  void *ctx),
...
#define TEST4_INITIALIZER0(n2k_id, Em, Im, Tm, Lm)                             \
	{                                                                      \
		.magic = TEST_4_CTX_MAGIC, .n2kafka_id = n2k_id,               \
		.expected_msgs = (Em), .interval_bytes_multiplier = (Im),      \
		.total_bytes_multiplier = (Tm), .limit_bytes_multiplier = (Lm) \
	}
...
static void validate_report(json_t *msg, void *vctx) {
	...
	const int rc = json_unpack_ex(msg,
				      &jerr,
				      0,
				      "{s:s,s:I,s:I,s:s,s:s}",
				      "n2kafka_id",
	...
	assert(client_limit_bytes ==
	       dsensor_uuid * ctx->limit_bytes_multiplier);
}

static void validate_test4_reports(struct kafka_message_array *ma, void *vctx) {
	struct test4_ctx *ctx = vctx;
...
static void validate_reset_bytes_consumed() {
	...
	const struct report_validation validators[] = {
			/* 1st step: Every client consume N bytes. we should
			check
			that bytes has been consumed and total_consumed_bytes
			has
			...
			/* 2nd step: Every client consume N bytes. We should
			check
			that bytes has been consumed and total_consumed_bytes
			has
			been updated accordly */
			...
			/* 3rd step: Clean phase. Every client should have 0
			bytes
			consumed in this interval, and 2*uuid
			...
			/* 4th step: Every client consume N bytes. We should
			check
			that bytes has been consumed, and total_consumed_bytes
			has
			been updated accordly */
			...
			/* 5th step: Any client consume, and we call for report.
			We
	...
	for (i = 0; i < n_uuids; ++i) {
		char uuid[BUFSIZ];
		snprintf(uuid, sizeof(uuid), "%zu", i);
		json_object_set_new(uuids,
				    uuid,
				    new_config_uuid(sensor_enrichment(uuid),
						    i * 10));
	}
...
static struct kafka_message_array *gen_client_consume_sync_bytes(
	...
	for (i = 0; i < n_uuids; ++i) {
		...
		const uint64_t value =
				i * (uint64_t) template->value_multiplier;
		...
		save_kafka_msg_key_partition_in_array(ret,
						      dup_key,
						      template->key_len,
						      msg,
						      strlen(msg),
...
#define TEST5_CONSUME_BYTES_INIT0(mtimestamp,                                  \
				  mn2kafka_id,                                 \
				  total_bytes_consumed_multiplier,             \
				  limit_bytes_multiplier,                      \
				  mvalue_multiplier,                           \
				  mpartition)                                  \
	{                                                                      \
		.timestamp = mtimestamp, .partition = mpartition,              \
		...
		.n2kafka_id = mn2kafka_id, .type = "data", .unit = "bytes",    \
		.key = TEST5_CONSUME_BYTES_KEY(mn2kafka_id, mtimestamp, 200),  \
		.key_len = sizeof(TEST5_CONSUME_BYTES_KEY(                     \
				mn2kafka_id, mtimestamp, 200))                 \
	}
#define TEST5_CONSUME_BYTES_INITP(mn2kafka_id,                                 \
				  total_bytes_consumed_multiplier,             \
				  limit_bytes_multiplier,                      \
				  mvalue_multiplier,                           \
				  mpartition)                                  \
	TEST5_CONSUME_BYTES_INIT0(1462282696,                                  \
				  mn2kafka_id,                                 \
				  total_bytes_consumed_multiplier,             \
				  limit_bytes_multiplier,                      \
				  mvalue_multiplier,                           \
				  mpartition)
#define TEST5_CONSUME_BYTES_INIT(mn2kafka_id,                                  \
				 total_bytes_consumed_multiplier,              \
				 limit_bytes_multiplier,                       \
				 mvalue_multiplier)                            \
	TEST5_CONSUME_BYTES_INITP(mn2kafka_id,                                 \
				  total_bytes_consumed_multiplier,             \
				  limit_bytes_multiplier,                      \
				  mvalue_multiplier,                           \
				  0)
	...
	static const struct client_consume_sync_bytes_template sync_bytes =
			TEST5_CONSUME_BYTES_INIT(
					TEST_OTHER_N2KAFKA_ID, 1, 10, 1);
	...
	struct kafka_message_array *sync_msgs =
			gen_client_consume_sync_bytes(n_uuids, &sync_bytes);
	struct kafka_message_array *my_sync_msgs =
			gen_client_consume_sync_bytes(n_uuids, &my_sync_bytes);

	const struct report_validation validators[] = {
			/* 1st step: Every client consume N bytes on others
			n2kafka.
			we should check that no report are given, since we don't
			have
			consumed anything */
			{
					.sync_msgs = sync_msgs,
			...
			/* 2nd step: Every client consume N bytes in this
			n2kafka.
			...
			/* 3rd step: Clean phase. We only receive our first
			consumed
			bytes, that we should ignore, so we have received
			nothing.
			*/
			{
...
static void validate_time_slices() {
	...
	struct {
		...
		int cmp_time_slice;
				     ...
				     .slice_width = 30,
				     .slice_off = 0,
				     .expected_slice_ts1 = 0,
				     .expected_slice_ts2 = 0,
				     .cmp_time_slice = 0,
		     },
		     {
				     .ts1 = 15,
				     ...
				     .slice_width = 60,
				     .slice_off = 0,
				     .expected_slice_ts1 = 1,
				     .expected_slice_ts2 = 1,
				     .cmp_time_slice = 0,
		     },
		     {
				     .ts1 = 15,
	...
	for (i = 0; i < RD_ARRAYSIZE(tests); ++i) {
		const int64_t time_slice_1 =
				timestamp_interval_slice(tests[i].ts1,
							 tests[i].slice_width,
							 tests[i].slice_off);
		const int64_t time_slice_2 =
				timestamp_interval_slice(tests[i].ts2,
							 tests[i].slice_width,
							 tests[i].slice_off);
...
#define TEST6_KAFKA_MSG_INITIALIZER0(                                          \
		m_err, m_partition, m_payload, m_len, m_key, m_key_len)        \
	{                                                                      \
		.err = m_err, .rkt = NULL, .partition = m_partition,           \
		.payload = m_payload ? memdup(m_payload, m_len) : NULL,        \
		.len = m_len, .key = m_key ? memdup(m_key, m_key_len) : NULL,  \
		.key_len = m_key_len, .offset = msg_offset++, ._private = NULL \
	}

#define TEST6_KAFKA_MSG_INITIALIZER(m_err, m_partition, m_payload, m_key)      \
	TEST6_KAFKA_MSG_INITIALIZER0(m_err,                                    \
				     m_partition,                              \
				     m_payload,                                \
				     sizeof(m_payload) - 1,                    \
				     m_key,                                    \
	...
	rd_kafka_message_t msgs[] = {
			...
			/* Invalid message: Object instead of string in monitor
			   value */
			TEST6_KAFKA_MSG_INITIALIZER(
			...
			TEST6_KAFKA_MSG_INITIALIZER(
					RD_KAFKA_RESP_ERR_NO_ERROR,
					test_partition,
					"{\"monitor\":\"organization_received_"
					"bytes2\","
					"\"value\":100,"
					"\"organization_uuid\":\"abc_org\","
					"\"n2kafka_id\":\"" TEST_N2KAFKA_ID
			...
			/* Invalid message: Object instead of integer in
			   timestamp
										value */
			TEST6_KAFKA_MSG_INITIALIZER(
					RD_KAFKA_RESP_ERR_NO_ERROR,
...
static void validate_thread_sync_partitions() {
	...
	struct kafka_message_array partition_eof_msg[] = {
			PARTITION_EOF_MSG_ARRAY(0), PARTITION_EOF_MSG_ARRAY(1),
	};
	...
	struct check_consumed_cb_ctx check_ctx[] = {
			...
			/* Partition 0 is in sync, so we only receive others 3
			   msgs */
			CHECK_CONSUMED_CB_CTX_INIT(10, 0x02020201, 0x02020201),
			/* Partition 1 is in sync, so we only receive others 2
			   msgs */
			CHECK_CONSUMED_CB_CTX_INIT(10, 0x03020301, 0x03020301),
	};

	const struct report_validation validators[] = {
			/* 1st step: No partition is in sync, we need to hear
			our
			own messages and any other messages */
			{
					...
					.org_db_cb_bef_report =
							check_consumed_cb,
					.org_db_cb_bef_report_ctx =
							&check_ctx[0],
			},
			/* 2nd step: Partition 0 sync. We need to stop listening
			our own
			messages in that partition, but keep listening our
			others
			partition messages */
			{
			...
			{
					/* We receive EOF_PARTITION again, since
					we have
					consumed all pending messages */
					.sync_msgs = &partition_eof_msg[0],
					.consume_ctx = &consume_ctx,
			},
			/* 3rd step: Partition 1 sync. We need to stop listening
			our own
