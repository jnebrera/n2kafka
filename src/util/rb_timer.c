...
static void
rb_timers_list_insert_head(rb_timers_list_t *list, struct rb_timer *elm) {
...
void rb_timers_run(rb_timers_list_t *list) {
	...
	rb_timers_list_foreach(timer, &list->list) {
		if (timer->execute_cb) {
			rdlog(LOG_DEBUG,
			      "Raising & restarting timer %p",
			      timer);

			rb_timer_t *pending_timer =
					calloc(1, sizeof(*pending_timer));
			if (NULL == pending_timer) {
				rdlog(LOG_ERR,
				      "Couldn't duplicate list element! %p",
				      timer);
			} else {
				memcpy(pending_timer,
				       timer,
