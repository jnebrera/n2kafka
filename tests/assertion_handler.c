...
struct assertion_handler_s *assertion_handler_new() {
	struct assertion_handler_s *assertion_handler =
			(struct assertion_handler_s *)malloc(
...
int assertion_handler_assert(struct assertion_handler_s *assertion_handler) {
	...
	TAILQ_FOREACH(assertion, &assertion_handler->assertion_q, tailq) {
		...
		json_t *assertion_json =
				json_loads(assertion->str, 0, &assertion_jerr);
		json_unpack_ex(assertion_json,
			       &assertion_jerr,
			       JSON_STRICT,
			       "{s:s}",
			       "message",
		...
		TAILQ_FOREACH(value, &assertion_handler->value_q, tailq) {
			...
			json_t *value_json =
					json_loads(value->str, 0, &value_jerr);
			json_unpack_ex(value_json,
				       &value_jerr,
				       JSON_STRICT,
				       "{s:s}",
				       "message",
			...
			// printf("%s:%s -> %d\n", assertion_message,
			// value_message,
			//        strcmp(assertion_message, value_message) ==
			//        0);
